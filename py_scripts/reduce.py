...
# constantscal_wave_4auto
c = 2.99792458 * 10 ** (8) # speed of light in m/s

# Spectral absorption line rest wavelengths
h_beta = 4861.363
h_gamma = 4340.472
h_delta = 4101.734
h_epsilon = 3970.075
...
def zap_two(input_dir, input_folder, file_number_list, 
    ...
    for i in range(np.shape(data_cube)[0]):
        ...
        # Now run through the length of l to see if any have 
        # large difference (max_diff)
...
def fit_lorentzian_quad_no_arc(spectrum, x, left, right, 
    ...
    if (plot == True):
        plt.figure(figsize = (figwidth, figheight))
        plt.plot(x, spectrum, color='k', linewidth=0.6, alpha=0.5)
        plt.plot(x, Lorentzian_quad(
            x, popt[0], popt[1], popt[2], popt[3], 
            popt[4], popt[5]), color='b')
        plt.ylim(ymin, ymax)
        lines(xmin, xmax)
        plt.xlim(xmin, xmax)
    velo = calc_velo(popt[2], line)
...
def fit_lorentzian_lin_no_arc(spectrum, x, left, right, 
                                 params, params_shift_night, 
                                 line, plot=True, 
                                 xmin=3500, xmax=6000, 
                                 ymin=-0.1, ymax=1.1, 
...
def fit_lorentzian_quad_pixel(spectrum, left, right, 
                              line, plot=True, xmin=3500, xmax=6000, 
                              ymin=-0.1, ymax=1.1, 
                              figwidth=10, figheight=3):
...
def fit_lorentzian_lin_pixel(spectrum, left, right, 
                             line, plot=True, 
                             xmin=3500, xmax=6000, 
                             ymin=-0.1, ymax=1.1, 
                             figwidth=10, figheight=3):
    ...
    sigma = np.sqrt(sum(spectrum[a:b] * (x[a:b] - mean) ** 2) 
                    / sum(spectrum[a:b]))
    popt, covar = opt.curve_fit(Lorentzian_lin, 
                                x[a:b], spectrum[a:b], 
                                p0 = [spectrum[a:b].max(), 
                                      sigma, mean, 0, 1])
    if (plot == True):
...
def file_to_spectrum(input_dir, input_folder, 
                     file_number_list, yearmonth=2308):
    fits_data = []
    for i in file_number_list:
        fits_data.append(fits.getdata(input_dir + str(yearmonth) 
...
def cal_wave_4auto(input, params):
    x = np.arange(len(input))
    lambda_ang = (params[0] * x ** 4 + params[1] * x ** 3 
                  + params[2] * x ** 2 + params[3] * x + params[4])
    return(lambda_ang)
...
def find_lorentzian_maxes_old(data, range=10):
    ...
    while (i < (l - 20) and i < 4040):
        ...
        p2 = data[i + 2]
        p3 = data[i + 3]
        if (    temp > m1 and temp > p1 and 
                temp >= m2 and temp >= p2 and 
                temp >= m3 and temp >= p3 and 
                temp >= m8 and temp > 1.3 * median):
    ...
    for wv in array1:
        if (wv != 3922):
            ...
            sigma = np.sqrt(sum(data[a:b] * (xlin - mean) ** 2) 
                            / sum(data[a:b]))
            max = np.max(data[a: b])
            mean_out = np.mean([data[a - 10:a], data[b:b + 10]])
            far_mean = np.mean(data[wv - 100:wv + 100])
            popt, covar = opt.curve_fit(Lorentzian_const, 
                                        xlin, data[a:b], 
                                        p0=[mean * 5, sigma, wv, 0])
            ...
            covar_array = np.append(covar_array, 
                                    np.array([np.array([temp1, temp2, 
                                                        temp3, temp4])]),
                                    axis=0)
...
def find_lorentzian_maxes(data, range=10):
    ...
    for wv in array1:
        ...
        if (max > 2 * far_mean):
            ...
            popt, covar = opt.curve_fit(gaussian_model, 
                                        xlin, data[a:b], 
                                        p0 = [mean * 2, sigma / 10, wv, 0])
            param_array = np.append(param_array, [popt], axis=0)
            temp1 = np.sqrt(covar[0][0])
            temp2 = np.sqrt(covar[1][1])
            temp3 = np.sqrt(covar[2][2])
...
def plot_maxes_lorntz(spectrum, maxes, params, minx=1000, 
                      figwidth=16 * 4, figheight=3, ymin=0, ymax=1):
    plt.figure(figsize=(figwidth, figheight))
    x = np.arange(len(spectrum))
    ...
    for i in range(len(maxes)):
        if (maxes[i] > minx):
            ...
            plt.plot(xtemp, 
                     Lorentzian_const(xtemp, params[i][0], 
...
def plot_maxes_lorntz_gauss(spectrum, maxes, 
                            params, fit_array, minx=1000, 
                            figwidth=16 * 4, figheight=3, 
                            ymin=0, ymax=1):
    plt.figure(figsize=(figwidth, figheight))
    x = np.arange(len(spectrum))
    plt.plot(x, spectrum, color='k', linewidth=1.5, alpha=0.3)
    for i in range(len(maxes)):
        if (maxes[i] > minx):
            if (fit_array[i] == 'Lorentzian'):
                xtemp = np.arange(maxes[i] - 8, maxes[i] + 8)
                plt.plot(xtemp, 
                         Lorentzian_const(xtemp, params[i][0], params[i][1],
                ...
                xtemp = np.arange(maxes[i] - 8, maxes[i] + 8)
                plt.plot(xtemp, 
                         gaussian_model(xtemp, params[i][0], params[i][1], 
                                        params[i][2], params[i][3]), 
                         color='b', alpha=0.5)
                plt.text(params[i][2], 2000, i, rotation=0, fontsize=13)
    plt.xlim(left = minx)
...
def plot_mins(spectrum, x, mins, minx=1, maxx=10000, 
              figwidth=15, figheight=3):
    plt.figure(figsize=(figwidth, figheight))
...
def plot_lines(lines, col, line_height=10, max_intensity=0, 
               figwidth=16 * 5, figheight=2, xmin=0, xmax=6000):
    max = np.max(lines[:, 2])
    plt.figure(figsize=(figwidth, figheight))
    for i in range(len(lines)):
        if (int(lines[i, 2]) > max_intensity and 
                lines[i, 0] < xmax and 
                lines[i, 0] > xmin):
            plt.axvline(lines[i, 0], ymax=line_height * (lines[i, 2] / max), 
                        color=col, linewidth=0.5, alpha=1)
...
def calc_velo_err(covar, wave_0):
    return (c * (np.sqrt(covar[2, 2]) * 10 ** (-10)) 
            / (wave_0 * 10 ** (-10)) / 1000)
...
def calc_velo(wave, wave_0):
    return (c * (wave * 10 ** (-10) - wave_0 * 10 ** (-10)) 
            / (wave_0 * 10 ** (-10)) / 1000)
...
def basic_plot(input_array, min = 75, max = 120, title = ''):
    ...
    plt.imshow(input_array, aspect = 'auto', vmin=min, vmax=max)
...
def plot_limits_size(input_array, min=75, max=120,
                     ...
                     marker=False, markerx=0, markery=0, markeralpha=1, 
                     markersize=10, markercolor='r'):
    plt.figure(figsize=(figsizewidth, figsizeheight))
    if (marker == True):
        plt.plot(markerx, markery, marker='o', 
                 alpha=markeralpha, markersize=markersize, 
                 color=markercolor, markeredgecolor='w')
    plt.imshow(input_array, aspect='auto', vmin=min, vmax=max)
    #, norm = LogNorm(vmin = 1, vmax = 1000000))
    plt.title(title)
...
def plot_limits_size_line(input_array, min=75, max=120, 
                          xleft=0, xright=4128, ybottom=1016, ytop=0, 
                          figsizewidth=12 * 1.4, figsizeheight=3 * 1.4, 
                          title='', marker=False, markerx=0, markery=0, 
                          markeralpha=1, markersize=10, markercolor='r', 
...
def fits_overscan_median(input_dir, input_folder, 
                         file_number_list, yearmonth=2308):
    fits_data = []
    for i in file_number_list:
        temp = fits.getdata(input_dir + str(yearmonth) 
                            + str(input_folder) + '/' + i 
                            + '.fits', ext=0).astype(np.float64)
        ...
        fits_data.append(overscan_corrected)
...
def fit_to_array(input_dir, input_folder, file_number, yearmonth=2308):
    fits_data = fits.getdata(input_dir + str(yearmonth) 
                        + str(input_folder) + '/' + file_number 
                        + '.fits', ext=0).astype(np.float64)
    return(fits_data)
...
def fits_overscan_normalize_median(input_dir, input_folder, 
                                   file_number_list, yearmonth=2308):
    fits_data = []
    for i in file_number_list:
        temp = fits.getdata(input_dir + str(yearmonth) 
        ...
        fits_data.append(overscan_corrected 
                         / np.median(overscan_corrected[:, 33:4096]))
    data_cube = np.dstack(fits_data)
    median_data = np.median(data_cube, axis=2)
    return(median_data)


def fits_overscan_rangenormalize_median(input_dir, input_folder, 
                                        file_number_list, min=0, max=1, 
                                        yearmonth=2308):
    fits_data = []
    for i in file_number_list:
        temp = fits.getdata(input_dir + str(yearmonth) 
